...
class TechnoeconomicSubmissionRequest(StrictTechnoeconomicRequest):
    calculation_contract_version: Literal[
        ...
        "tea-calculation-v4", "tea-calculation-v5",
    ...
    def validate_submission_contract(self) -> "TechnoeconomicSubmissionRequest":
        inferred_contract_version = (
            "tea-calculation-v5" if self.paired_commercial is not None else
            "tea-calculation-v4" if self.standalone_commercial is not None else
            "tea-calculation-v3" if self.commercial_scaling is not None else
            "tea-calculation-v2" if self.capacity_normalization is not None else
            "tea-calculation-v1"
        )
        if (self.calculation_contract_version is not None
