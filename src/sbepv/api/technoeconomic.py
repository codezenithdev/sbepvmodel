...
def canonical_submission_request_payload(
    ...
    """Preserve the historical durable request shape for each supported workflow."""
    request = _parsed_submission_request(request_payload)
    canonical = request.model_dump(mode="json", exclude_none=False)
    for field in ("capacity_normalization", "commercial_scaling", "standalone_commercial", "paired_commercial"):
