...
class RetiredWorkflow(InvalidStateTransition):
    """Raised when saved work belongs to a removed workflow."""


class JobCompletionCancelled(InvalidStateTransition):
    """Raised when cancellation wins model-job finalization."""
...
class AgentStore:
        ...
        with self._lock:
            ...
            try:
                ...
                if version < 5:
                    ...
                    version = 5
                # Versions 6–9 belonged to retired features. Reserve those numbers
                # without creating their tables; keep existing archives untouched.
                if version < SCHEMA_VERSION:
                    connection.execute("BEGIN IMMEDIATE")
                    for reserved_version in range(version + 1, SCHEMA_VERSION + 1):
                        connection.execute(
                            "INSERT OR IGNORE INTO schema_migrations(version, applied_at) VALUES (?, ?)",
                            (reserved_version, _timestamp(self._current_time())),
                        )
    ...
    @staticmethod
    def _archived_reference_exists(
        connection: sqlite3.Connection, table: str, column: str, value: str,
    ) -> bool:
        """Protect immutable references in optional historical tables."""
        allowed = {
            ("decision_scenario_jobs", "tea_job_id"),
            ("decision_cases", "source_annual_job_id"),
        }
    ...
    def _ensure_technoeconomic_row_supported(
        self, connection: sqlite3.Connection, row: sqlite3.Row,
    ) -> None:
        request = _json_load(row["request_json"]) or {}
        paired = request.get("paired_commercial") or {}
        if (request.get("calculation_contract_version") == "tea-calculation-v6"
                or paired.get("lifecycle") is not None):
            raise RetiredWorkflow("TEA v6 has been retired; create a new v5 analysis.")
        if self._archived_reference_exists(
            connection, "decision_scenario_jobs", "tea_job_id", row["tea_job_id"],
        ):
            raise RetiredWorkflow("Autonomy jobs have been retired; create a new manual TEA analysis.")

    def ensure_technoeconomic_job_supported(self, job_id: str) -> None:
        """Reject retired saved work before reads, retries, or calculation."""
        with self._transaction() as connection:
            ...
            self._ensure_technoeconomic_row_supported(connection, row)
    ...
    def update_job(
        ...
        *,
        ...
        with self._transaction(write=True) as connection:
            ...
            if retained_payload_update and (
                retained_source is not None or self._archived_reference_exists(
                    connection, "decision_cases", "source_annual_job_id", job_id,
                )
            ):
                raise InvalidStateTransition(
        ...
        *,
        ...
        with self._transaction(write=True) as connection:
            ...
            if row is None:
                raise RecordNotFound(f"unknown technoeconomic job: {job_id}")
            if self._archived_reference_exists(
                connection, "decision_scenario_jobs", "tea_job_id", job_id,
            ):
                raise InvalidStateTransition("This job is retained by archived Autonomy records.")
    ...
    def delete_job(self, job_id: str) -> dict[str, Any]:
        ...
        with self._transaction(write=True) as connection:
            ...
            if self._archived_reference_exists(
                ...
                raise InvalidStateTransition("This Annual source is retained by archived Autonomy records.")
