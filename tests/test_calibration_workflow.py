...
class CalibrationWorkflowTests(unittest.TestCase):
    ...
    def test_historian_parser_bounds_temperature_fallback_and_fit_mask(
        self,
    ) -> None:
        frame = self._valid_historian_frame(
            [
        ...
        self.assertEqual(
            parsed["calibration_fit_eligible"].tolist(),
    ...
    def test_measured_curtailment_rows_are_excluded_from_factor_fit(self) -> None:
        index = pd.date_range(
            "2026-06-01 12:00:00",
            periods=2,
            freq="h",
        ...
        for system, factor_column in (
            ...
            with self.subTest(system=system):
                observation = calibration["overall_period"][system]
                self.assertEqual(observation["sample_count"], 1)
                self.assertEqual(observation["factor"], 1.0)
                self.assertEqual(calibrated[factor_column].tolist(), [1.0, 1.0])

    def test_calibration_fit_eligibility_excludes_interpolated_rows(self) -> None:
        index = pd.date_range(
            "2026-06-01 12:00:00",
            periods=2,
            freq="h",
